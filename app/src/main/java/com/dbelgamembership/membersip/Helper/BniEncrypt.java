...
public class BniEncrypt {
    ...
    public static String hashData(String json_data, String cid, String secret) {
        ...
        String hash = doubleEncrypt(String.valueOf(strrevtime) + "." + json_data, cid, secret);
        return hash;
    }
    ...
    protected static Boolean tsDiff(int ts) {
        return (Math.abs(ts - Integer.parseInt(getTime())) <= 300) ? Boolean.valueOf(true) : Boolean.valueOf(false);
    }
    ...
    protected static String doubleDecrypt(String string, String cid, String secret) {
        Double ceils = Math.ceil(string.length() / 4.0D) * 4.0D;
