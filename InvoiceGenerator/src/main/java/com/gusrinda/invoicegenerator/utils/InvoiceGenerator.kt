...
class InvoiceGenerator(private val context: Context) {
    ...
    private var invoicePaymentInfo = ArrayList<ModelInvoicePaymentInfo>();
    ...
    fun setInvoicePaymentInfo(invoicePaymentInfo: List<ModelInvoicePaymentInfo>) {
        this.invoicePaymentInfo = invoicePaymentInfo as ArrayList<ModelInvoicePaymentInfo>
    }
...
/*
    ...
    private fun initItemsTable(doc: Document) {
        ...
        for (item in invoiceTableData) {
            ...
            val disAmount = PdfPCell(Phrase(item.thirdItem.toString(), appFontRegular))
            disAmount.border = Rectangle.NO_BORDER
            disAmount.horizontalAlignment = Rectangle.ALIGN_RIGHT
            disAmount.paddingTop = TABLE_TOP_PADDING
            ...
            val netAmount = PdfPCell(Phrase(item.fifthItem.toString(), appFontRegular))
            netAmount.horizontalAlignment = Rectangle.ALIGN_RIGHT
            netAmount.border = Rectangle.NO_BORDER
            netAmount.paddingTop = TABLE_TOP_PADDING
            netAmount.paddingRight = PADDING_EDGE
    ...
    private fun initPriceDetails(doc: Document) {
        ...
        val txtDiscCell = PdfPCell(Phrase("Discount Total : ", appFontRegular))
        txtDiscCell.border = Rectangle.NO_BORDER
        txtDiscCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        txtDiscCell.paddingTop = TEXT_TOP_PADDING
        priceDetailsTable.addCell(txtDiscCell)

        val totalDiscCell = PdfPCell(
            Phrase(
                "$invoiceCurrency ${invoicePriceDetailsDataSource.discountTotal}",
                appFontBold
            )
        )
        totalDiscCell.border = Rectangle.NO_BORDER
        totalDiscCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        totalDiscCell.paddingTop = TEXT_TOP_PADDING
        totalDiscCell.paddingRight = PADDING_EDGE
        priceDetailsTable.addCell(totalDiscCell)

        //ADDING VOUCHER MEMBER

        if (invoicePriceDetailsDataSource.isUsingVoucherMember) {

            val txtVoucherMember = PdfPCell(Phrase("Voucher MBR ${invoicePriceDetailsDataSource.kodeVoucherMember} : ", appFontRegular))
            txtVoucherMember.border = Rectangle.NO_BORDER
            txtVoucherMember.horizontalAlignment = Rectangle.ALIGN_RIGHT
            txtVoucherMember.paddingTop = TEXT_TOP_PADDING
            priceDetailsTable.addCell(txtVoucherMember)

            val totalVoucherMemberCell = PdfPCell(
                Phrase(
                    "$invoiceCurrency ${invoicePriceDetailsDataSource.nominalVoucherMember}",
                    appFontBold
                )
            )
            totalVoucherMemberCell.border = Rectangle.NO_BORDER
            totalVoucherMemberCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
            totalVoucherMemberCell.paddingTop = TEXT_TOP_PADDING
            totalVoucherMemberCell.paddingRight = PADDING_EDGE
            priceDetailsTable.addCell(totalVoucherMemberCell)
        }

        //ADDING VOUCHER SUPLIER
        if (invoicePriceDetailsDataSource.isUsingVoucherSuplier) {

            val txtVoucherSuplier = PdfPCell(Phrase("Voucher SP ${invoicePriceDetailsDataSource.kodeVoucherSuplier} : ", appFontRegular))
            txtVoucherSuplier.border = Rectangle.NO_BORDER
            txtVoucherSuplier.horizontalAlignment = Rectangle.ALIGN_RIGHT
            txtVoucherSuplier.paddingTop = TEXT_TOP_PADDING
            priceDetailsTable.addCell(txtVoucherSuplier)

            val totalVoucherSuplierCell = PdfPCell(
                Phrase(
                    "$invoiceCurrency ${invoicePriceDetailsDataSource.nominalVoucherSuplier}",
                    appFontBold
                )
            )
            totalVoucherSuplierCell.border = Rectangle.NO_BORDER
            totalVoucherSuplierCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
            totalVoucherSuplierCell.paddingTop = TEXT_TOP_PADDING
            totalVoucherSuplierCell.paddingRight = PADDING_EDGE
            priceDetailsTable.addCell(totalVoucherSuplierCell)
        }

        val txtShipCell = PdfPCell(Phrase("Shipment Total : ", appFontRegular))
        txtShipCell.border = Rectangle.NO_BORDER
        txtShipCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        txtShipCell.paddingTop = TEXT_TOP_PADDING
        priceDetailsTable.addCell(txtShipCell)

        val totalShipCell = PdfPCell(
            Phrase(
                "$invoiceCurrency ${invoicePriceDetailsDataSource.ongkosTotal}",
                appFontBold
            )
        )
        totalShipCell.border = Rectangle.NO_BORDER
        totalShipCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        totalShipCell.paddingTop = TEXT_TOP_PADDING
        totalShipCell.paddingRight = PADDING_EDGE
        priceDetailsTable.addCell(totalShipCell)
        ...
        val lineSplitCell = PdfPCell(Phrase("+ ", appFontRegular))
        lineSplitCell.border = Rectangle.NO_BORDER
        lineSplitCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        lineSplitCell.paddingTop = TEXT_TOP_PADDING
        lineSplitCell.paddingBottom = TEXT_TOP_PADDING
        lineSplitCell.paddingLeft = PADDING_EDGE
        priceDetailsTable.addCell(lineSplitCell)
        ...
        lineCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        lineCell.paddingTop = TEXT_TOP_PADDING
        lineCell.paddingBottom = TEXT_TOP_PADDING
        lineCell.paddingRight = PADDING_EDGE
        priceDetailsTable.addCell(
            PdfPCell(
                lineCell
            )
        )
    ...
    private fun initPaymentDetails(doc: Document) {
        val priceDetailsTable = PdfPTable(2)
        priceDetailsTable.totalWidth = PageSize.A4.width
        priceDetailsTable.setWidths(floatArrayOf(5f, 2f))
        priceDetailsTable.isLockedWidth = true
        ...
        for (payment in invoicePaymentInfo) {

            val txtSubTotalCell = PdfPCell(Phrase("${payment.tipePayment} ", appFontRegular))
            txtSubTotalCell.border = Rectangle.NO_BORDER
            txtSubTotalCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
            txtSubTotalCell.paddingTop = TEXT_TOP_PADDING
            priceDetailsTable.addCell(txtSubTotalCell)
            val totalPriceCell = PdfPCell(
                Phrase(
                    "$invoiceCurrency ${payment.amountPayment}",
                    appFontBold
                )
            )
            totalPriceCell.border = Rectangle.NO_BORDER
            totalPriceCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
            totalPriceCell.paddingTop = TEXT_TOP_PADDING
            totalPriceCell.paddingRight = PADDING_EDGE
            priceDetailsTable.addCell(totalPriceCell)
        }

        val txtSubTotalCell = PdfPCell(Phrase("Change Total : ", appFontRegular))
        txtSubTotalCell.border = Rectangle.NO_BORDER
        txtSubTotalCell.horizontalAlignment = Rectangle.ALIGN_RIGHT
        txtSubTotalCell.paddingTop = TEXT_TOP_PADDING
        priceDetailsTable.addCell(txtSubTotalCell)
        val totalPriceCell = PdfPCell(
            Phrase(
                "$invoiceCurrency ${invoicePriceDetailsDataSource.changeTotal}",
                appFontBold
            )
        )
        totalPriceCell.border = Rectangle.NO_BORDER
